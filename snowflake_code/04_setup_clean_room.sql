...
RETURNS TABLE (AGE_BUCKET VARCHAR, TOTAL_TXN_AMOUNT FLOAT, TOTAL_CLAIM_AMOUNT FLOAT, RISK_SCORE FLOAT)
LANGUAGE SQL
EXECUTE AS OWNER
AS
$$
    ...
    res := (
        SELECT 
            CASE 
                ...
                ELSE '70+' 
            END AS AGE_BUCKET,
            CAST(SUM(b.AMOUNT) AS FLOAT) AS TOTAL_TXN_AMOUNT,
            CAST(SUM(i.CLAIM_AMOUNT) AS FLOAT) AS TOTAL_CLAIM_AMOUNT,
            -- Simple Risk Score
            CAST((SUM(CASE WHEN b.IS_FRAUDULENT THEN 1 ELSE 0 END) + 
             SUM(CASE WHEN i.STATUS = 'Rejected' THEN 1 ELSE 0 END)) AS FLOAT) AS RISK_SCORE
